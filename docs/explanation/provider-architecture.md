...
Wire literals already defined by those SDK contracts remain vendor-owned. Each
adapter accepts only the native request, response, and stream variants declared
by that adapter; catalog presence does not widen the contract.
...
The pinned serializer does not support arbitrary partial field selection;
callers use the declared snapshot contract.
...
it does not replace the durable Agent-run lifecycle. The Redis mutex is an
admission guard, not durable lifecycle authority.
...
Legacy Hume language/sample-rate inputs are not native selectors. The onboarding
surface does not expose library-versus-custom voice selection.
...
conversion happen without an intervening task scheduling gap. See the
[SoXR final-chunk contract](https://python-soxr.readthedocs.io/en/latest/soxr.html#soxr.ResampleStream.resample_chunk).
...
not recorded. Response-ID correlation, reconnect continuity, and native carrier
envelopes remain adapter-owned. Browser and WebRTC session registries are
process-local rather than cross-worker authority.

The generic `TTSConfig` carrier holds open option/format mappings. Each adapter
must validate its native options, requests, responses, and lifecycle behavior
before provider I/O.
...
Adapter-specific lifecycle behavior remains explicit in each factory; the shared
manager does not infer unsupported native behavior.
...
stream recovery is not unified: the runtime has its send-retry policy,
the task supervisor has restart rules, and some native adapters reconnect
internally. The runtime therefore does not promise serialized reconnect ownership
or safe replay after an uncertain write. As with Python's [asyncio timeout contract](https://docs.python.org/3.13/library/asyncio-task.html#timeouts),
...
closes resources, not a guaranteed final-output drain.
...
protocol refusals after a successful upgrade and adapter-internal reconnect loops
do not become safe replay automatically. HTTP classification is not proof that
every native close releases all resources.
...
guarantees, not durable delivery across process death.
...
close retains final output within a bounded drain. Whole-call teardown can report
incomplete finalization rather than claim canonical persistence. Protocol details
stay in the
...
is not implicitly closed; the caller explicitly owns that transaction boundary.
...
source-history path. Swarm shared dispatch and transcript persistence keep their
existing transaction owners; this context adapter does not merge them.
...
String tool-result serialization is vendor-neutral and retains the existing TOON
representation.
...
every eagerly evaluated type-alias failure. Native request/response behavior is
vendor-owned and must pass the provider verification action.
