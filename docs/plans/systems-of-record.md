...
- External knowledge: spaces, documents, blocks, versions, attachments.
...
- identity: `id`, `organization_id`, human name
...
- source, optional stream, kind (`SCHEMA | BOOTSTRAP | INCREMENTAL |
...
5. Product sync rows commit before `sor.bootstrap_source` is enqueued.
6. Bootstrap pages records into the canonical projection and checkpoints each
   committed page.
...
10. A complete full-scan reconciliation may tombstone unseen records only after
...
summaries. Files are metadata-only in the first slice unless content extraction
is explicitly supported; OCR and URL crawling remain out of scope.
...
| Vendor | Canonical sources | Change strategy | Custom data | Auth |
| --- | --- | --- | --- | --- |
| Confluence Cloud | spaces, pages, page bodies, versions, properties, attachments | full reconciliation | content properties retained for audit | Atlassian OAuth 2.0 3LO |
...
Agent view applies the same source grant, active mapping, field visibility, and
tool result projection used by runtime code. It is not a separately assembled
frontend approximation. This lets humans audit what an Agent could actually
observe.
...
controls dumped into one form. Submit and “Start new” clear local drafts;
refresh retains an unfinished draft. Active data remains visible while a new
mapping revision is drafted.
