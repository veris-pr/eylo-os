...
carrier applets. Both media routes require authenticated routing and the one-time
DB claim. Carrier catalog membership is not evidence of a working outbound call;
the configured provider must be verified.
...
Catalog presence does not establish endpoint availability or handshake latency;
verify the provider config before enabling it. A newer model or endpoint is not
selected implicitly.
...
Local contract probes cover these paths. Verify the configured endpoint, model,
and voice through the provider action before enabling the config.
