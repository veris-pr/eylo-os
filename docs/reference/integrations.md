...
querying are also typed as described below.
...
field would be a compatibility-breaking contract change.
...
default is `2022-11-28`; the consumed contracts follow its
...
The adapter does not support `2026-03-10`; see
...
Authenticated reads and mutations require their corresponding token permissions;
access to a public project does not establish those permissions.

### Jira request and response contracts

All four curated Jira tools use vendor-owned Cloud REST v3 models. They retain
the instance-specific `/rest/api/3` route and existing auth configuration. Jira
Data Center is not supported by this adapter.
...
already part of the vendor OAuth configuration.
...
claim batch-read optimization or exhaustive history.
