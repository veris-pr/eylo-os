"""Create the complete Eylo schema from an empty PostgreSQL database.

Revision ID: eylo0001
Revises: none
Create Date: 2026-09-14

This baseline intentionally replaces the repository's pre-release migration
history. Foreign keys are installed after every table because the schema has
deliberate aggregate cycles. Absurd's pinned 0.4.0 schema and upstream 0.5.0
migration are installed together so a fresh database starts at the runtime's
required durable-execution version.
"""
...
_ABSURD_0_5_MIGRATION_PATH = _VENDOR_DIRECTORY / "absurd-0.4.0-0.5.0.sql"
_DEFERRED_FOREIGN_KEYS: list[tuple[str, str | None, sa.ForeignKeyConstraint]] = []


class _VectorType(sa.types.UserDefinedType):
    """Migration-local pgvector type; keep the baseline self-contained."""
...
def _create_table(table_name: str, *elements: Any, **kwargs: Any) -> Any:
    """Create one table now and defer its foreign keys until all targets exist."""
    foreign_keys = tuple(
...
def _create_deferred_foreign_keys() -> None:
    """Install the frozen FK graph after every referenced table exists."""
...
def _install_absurd_schema() -> None:
    """Install the pinned Absurd schema, upgrade it to 0.5.0, then add our queue."""
    schema_sql = _ABSURD_0_4_SCHEMA_PATH.read_text(encoding="utf-8")
    schema_sql += "\n" + _ABSURD_0_5_MIGRATION_PATH.read_text(encoding="utf-8")
    schema_sql += (
...
def _drop_public_foreign_keys() -> None:
    """Drop public FKs before tables whose dependency graph contains cycles."""
    op.execute(
        """
        DO $$
...
def _drop_public_enum_types() -> None:
    """Remove enum types left behind after the public tables are gone."""
    op.execute(
        """
        DO $$
...
def upgrade() -> None:
    ...
    _create_table(
        ...
        sa.Column("waiting_tool_owner_id", sa.UUID(), nullable=True),
        sa.Column(
        ...
        sa.CheckConstraint(
            "(lifecycle = 'waiting_for_tool' AND waiting_tool_owner_kind ~ '^[a-z][a-z0-9_]{0,63}$' AND waiting_tool_owner_id IS NOT NULL) OR (lifecycle <> 'waiting_for_tool' AND waiting_tool_owner_kind IS NULL AND waiting_tool_owner_id IS NULL)",
        ...
        sa.CheckConstraint(
            "lifecycle NOT IN ('waiting_for_input', 'waiting_for_approval', 'waiting_for_tool') OR waiting_at IS NOT NULL",
    ...
    _create_table(
        ...
        sa.Column(
            "entry_channel",
            postgresql.ENUM(
                "widget", "telephony", "api", name="user_session_entry_channel_enum"
            ),
            nullable=False,
        ),
        sa.Column(
        ...
        sa.CheckConstraint(
            "(state = 'active' AND disconnected_at IS NULL AND ended_at IS NULL AND end_reason IS NULL) OR (state = 'disconnected' AND disconnected_at IS NOT NULL AND ended_at IS NULL AND end_reason IS NULL) OR (state IN ('ended', 'failed') AND ended_at IS NOT NULL AND end_reason IS NOT NULL)",
            name="ck_user_sessions_lifecycle",
        ),
        sa.CheckConstraint(
            "end_reason IS NULL OR (length(end_reason) BETWEEN 1 AND 128 AND end_reason ~ '^[a-z][a-z0-9_.-]*$')",
            name="ck_user_sessions_end_reason",
        ),
        sa.CheckConstraint(
            "connection_sequence >= 1", name="ck_user_sessions_connection_sequence"
        ),
        sa.CheckConstraint(
            "last_activity_at >= started_at AND (disconnected_at IS NULL OR disconnected_at >= started_at) AND (ended_at IS NULL OR ended_at >= started_at)",
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "id", "organization_id", name="uq_user_sessions_id_organization_id"
        ),
    )
    op.create_index(
    ...
    op.create_index(
        op.f("ix_user_sessions_state"), "user_sessions", ["state"], unique=False
    )
    _create_table(
        "voice_configs",
        sa.Column("name", sa.String(length=128), nullable=False),
        ...
        sa.CheckConstraint("revision > 0", name="ck_voice_configs_revision_positive"),
        sa.ForeignKeyConstraint(
            ["organization_id"], ["organization_organizations.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "id", "organization_id", name="uq_voice_configs_id_organization_id"
        ),
    )
    op.create_index(
    ...
    _create_table(
        ...
        sa.CheckConstraint(
            "jsonb_typeof(requested_scopes) = 'array' AND octet_length(requested_scopes::text) <= 65536",
    ...
    _create_table(
        ...
        sa.Column("webhook_endpoint_key", sa.UUID(), nullable=True),
        sa.Column(
        ...
        sa.Column(
            "webhook_authorized_connection_revision", sa.Integer(), nullable=True
        ),
        ...
        sa.CheckConstraint(
            "webhook_authorized_connection_revision IS NULL OR webhook_authorized_connection_revision > 0",
    ...
    _create_table(
        "sor_sources",
        sa.Column("name", sa.String(length=160), nullable=False),
        sa.Column("onboarding_attempt_id", sa.UUID(), nullable=False),
        sa.Column(
            "profile",
            postgresql.ENUM(
                "crm", "ticketing", "support", "knowledge", name="sor_profile_enum"
    ...
    _create_table(
        ...
        sa.CheckConstraint(
            "jsonb_typeof(depends_on) = 'array' AND octet_length(depends_on::text) <= 65536",
            name="ck_sor_source_streams_dependencies",
        ),
        sa.CheckConstraint(
            "jsonb_typeof(relationship_targets) = 'object' AND octet_length(relationship_targets::text) <= 65536",
    ...
    _create_table(
        ...
        sa.CheckConstraint(
            "(status = 'active' AND ended_at IS NULL AND ended_reason IS NULL AND duration_ms IS NULL) OR (status IN ('completed', 'failed') AND ended_at IS NOT NULL AND ended_reason IS NOT NULL AND duration_ms >= 0)",
    ...
    _create_table(
        ...
        sa.Column("relationship_role", sa.String(length=160), nullable=False),
        sa.Column(
    ...
    _create_table(
        ...
        sa.Column(
            "resolution_attempts", sa.Integer(), server_default="0", nullable=False
        ),
        sa.Column("last_attempt_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("last_error_code", sa.String(length=128), nullable=True),
        sa.Column("tombstoned_at", sa.DateTime(timezone=True), nullable=True),
        ...
        sa.CheckConstraint(
            "from_vendor_object_key <> to_vendor_object_key OR from_vendor_external_id <> to_vendor_external_id",
            name="ck_sor_relation_intents_distinct_endpoints",
        ),
        sa.CheckConstraint(
            "length(btrim(from_vendor_object_key)) > 0 AND length(btrim(from_vendor_external_id)) > 0 AND length(btrim(to_vendor_object_key)) > 0 AND length(btrim(to_vendor_external_id)) > 0",
    ...
    op.create_index(
        op.f("ix_sor_relation_intents_created_at"),
        "sor_relation_intents",
        ["created_at"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_sor_relation_intents_organization_id"),
        "sor_relation_intents",
        ["organization_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_sor_relation_intents_origin_record_id"),
        "sor_relation_intents",
        ["origin_record_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_sor_relation_intents_source_id"),
        "sor_relation_intents",
        ["source_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_sor_relation_intents_state"),
        "sor_relation_intents",
        ["state"],
        unique=False,
    )
    ...
    _create_table(
        ...
        sa.CheckConstraint(
            "normalized_state IS NULL OR normalized_state IN ('ACTIVE', 'ACHIEVED', 'BREACHED', 'PAUSED', 'UNAVAILABLE', 'UNKNOWN')",
    ...
    _create_table(
        ...
        sa.CheckConstraint(
            "normalized_status IS NULL OR normalized_status IN ('NEW', 'OPEN', 'PENDING', 'HOLD', 'RESOLVED', 'CLOSED', 'UNKNOWN')",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_unq_sor_ticketing_workflow_states_ext_id_org_id",
        table_name="sor_ticketing_workflow_states",
    )
    op.drop_index(
        op.f("ix_sor_ticketing_workflow_states_source_id"),
        table_name="sor_ticketing_workflow_states",
    ...
    op.drop_table("sor_ticketing_workflow_states")
    op.drop_index(
        "ix_unq_sor_ticketing_users_ext_id_org_id", table_name="sor_ticketing_users"
    )
    op.drop_index(
        "ix_sor_ticketing_users_source_name", table_name="sor_ticketing_users"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_users_source_id"), table_name="sor_ticketing_users"
    )
    op.drop_index(
        "ix_sor_ticketing_users_source_email", table_name="sor_ticketing_users"
    )
    op.drop_index(
        "ix_sor_ticketing_users_source_active", table_name="sor_ticketing_users"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_users_record_id"), table_name="sor_ticketing_users"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_users_organization_id"), table_name="sor_ticketing_users"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_users_created_at"), table_name="sor_ticketing_users"
    )
    op.drop_table("sor_ticketing_users")
    op.drop_index(
        "ix_unq_sor_ticketing_projects_ext_id_org_id",
        table_name="sor_ticketing_projects",
    )
    op.drop_index(
        "ix_sor_ticketing_projects_source_name", table_name="sor_ticketing_projects"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_projects_source_id"), table_name="sor_ticketing_projects"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_projects_record_id"), table_name="sor_ticketing_projects"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_projects_organization_id"),
        table_name="sor_ticketing_projects",
    ...
    op.drop_index(
        op.f("ix_sor_ticketing_projects_key"), table_name="sor_ticketing_projects"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_projects_created_at"),
        table_name="sor_ticketing_projects",
    )
    op.drop_table("sor_ticketing_projects")
    op.drop_index(
        "ix_unq_sor_ticketing_labels_ext_id_org_id", table_name="sor_ticketing_labels"
    )
    op.drop_index(
        "ix_sor_ticketing_labels_source_project", table_name="sor_ticketing_labels"
    )
    op.drop_index(
        "ix_sor_ticketing_labels_source_parent", table_name="sor_ticketing_labels"
    )
    op.drop_index(
        "ix_sor_ticketing_labels_source_name", table_name="sor_ticketing_labels"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_labels_source_id"), table_name="sor_ticketing_labels"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_labels_record_id"), table_name="sor_ticketing_labels"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_labels_organization_id"),
        table_name="sor_ticketing_labels",
    )
    op.drop_index(
        op.f("ix_sor_ticketing_labels_created_at"), table_name="sor_ticketing_labels"
    )
    op.drop_table("sor_ticketing_labels")
    op.drop_index(
        "ix_unq_sor_ticketing_issues_ext_id_org_id", table_name="sor_ticketing_issues"
    )
    op.drop_index(
        "ix_sor_ticketing_issues_source_team", table_name="sor_ticketing_issues"
    )
    op.drop_index(
        "ix_sor_ticketing_issues_source_status", table_name="sor_ticketing_issues"
    )
    op.drop_index(
        "ix_sor_ticketing_issues_source_project", table_name="sor_ticketing_issues"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_issues_source_id"), table_name="sor_ticketing_issues"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_issues_record_id"), table_name="sor_ticketing_issues"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_issues_organization_id"),
        table_name="sor_ticketing_issues",
    )
    op.drop_index(
        op.f("ix_sor_ticketing_issues_key"), table_name="sor_ticketing_issues"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_issues_created_at"), table_name="sor_ticketing_issues"
    )
    op.drop_table("sor_ticketing_issues")
    op.drop_index(
        "ix_unq_sor_ticketing_issue_relations_ext_id_org_id",
        table_name="sor_ticketing_issue_relations",
    )
    op.drop_index(
        "ix_sor_ticketing_issue_relations_source_to",
        table_name="sor_ticketing_issue_relations",
    ...
    op.drop_table("sor_ticketing_issue_relations")
    op.drop_index(
        "ix_unq_sor_ticketing_cycles_ext_id_org_id", table_name="sor_ticketing_cycles"
    )
    op.drop_index(
        "ix_sor_ticketing_cycles_source_project", table_name="sor_ticketing_cycles"
    )
    op.drop_index(
        "ix_sor_ticketing_cycles_source_name", table_name="sor_ticketing_cycles"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_cycles_source_id"), table_name="sor_ticketing_cycles"
    )
    op.drop_index(
        "ix_sor_ticketing_cycles_source_dates", table_name="sor_ticketing_cycles"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_cycles_record_id"), table_name="sor_ticketing_cycles"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_cycles_organization_id"),
        table_name="sor_ticketing_cycles",
    )
    op.drop_index(
        op.f("ix_sor_ticketing_cycles_created_at"), table_name="sor_ticketing_cycles"
    )
    op.drop_table("sor_ticketing_cycles")
    op.drop_index(
        "ix_unq_sor_ticketing_comments_ext_id_org_id",
        table_name="sor_ticketing_comments",
    )
    op.drop_index(
        "ix_sor_ticketing_comments_source_issue_created",
        table_name="sor_ticketing_comments",
    ...
    op.drop_index(
        op.f("ix_sor_ticketing_comments_source_id"), table_name="sor_ticketing_comments"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_comments_record_id"), table_name="sor_ticketing_comments"
    )
    op.drop_index(
        op.f("ix_sor_ticketing_comments_organization_id"),
        table_name="sor_ticketing_comments",
    ...
    op.drop_table("sor_ticketing_comments")
    op.drop_index(
        "ix_unq_sor_support_tickets_ext_id_org_id", table_name="sor_support_tickets"
    )
    op.drop_index(
        "ix_sor_support_tickets_source_status", table_name="sor_support_tickets"
    )
    op.drop_index(
        "ix_sor_support_tickets_source_requester", table_name="sor_support_tickets"
    )
    op.drop_index(
        op.f("ix_sor_support_tickets_source_id"), table_name="sor_support_tickets"
    )
    op.drop_index(
        "ix_sor_support_tickets_source_assignee", table_name="sor_support_tickets"
    )
    op.drop_index(
        op.f("ix_sor_support_tickets_record_id"), table_name="sor_support_tickets"
    )
    op.drop_index(
        op.f("ix_sor_support_tickets_organization_id"), table_name="sor_support_tickets"
    )
    op.drop_index(
        op.f("ix_sor_support_tickets_created_at"), table_name="sor_support_tickets"
    )
    op.drop_table("sor_support_tickets")
    op.drop_index(
        "ix_unq_sor_support_tags_ext_id_org_id", table_name="sor_support_tags"
    )
    op.drop_index("ix_sor_support_tags_source_name", table_name="sor_support_tags")
    op.drop_index(op.f("ix_sor_support_tags_source_id"), table_name="sor_support_tags")
    op.drop_index(op.f("ix_sor_support_tags_record_id"), table_name="sor_support_tags")
    op.drop_index(
        op.f("ix_sor_support_tags_organization_id"), table_name="sor_support_tags"
    )
    op.drop_index(op.f("ix_sor_support_tags_created_at"), table_name="sor_support_tags")
    op.drop_table("sor_support_tags")
    op.drop_index(
        "ix_unq_sor_support_sla_metrics_ext_id_org_id",
        table_name="sor_support_sla_metrics",
    )
    op.drop_index(
        "ix_sor_support_sla_metrics_source_ticket", table_name="sor_support_sla_metrics"
    )
    op.drop_index(
        op.f("ix_sor_support_sla_metrics_source_id"),
        table_name="sor_support_sla_metrics",
    ...
    op.drop_table("sor_support_sla_metrics")
    op.drop_index(
        "ix_unq_sor_support_queues_ext_id_org_id", table_name="sor_support_queues"
    )
    op.drop_index("ix_sor_support_queues_source_name", table_name="sor_support_queues")
    op.drop_index(
        op.f("ix_sor_support_queues_source_id"), table_name="sor_support_queues"
    )
    op.drop_index(
        op.f("ix_sor_support_queues_record_id"), table_name="sor_support_queues"
    )
    op.drop_index(
        op.f("ix_sor_support_queues_organization_id"), table_name="sor_support_queues"
    )
    op.drop_index(
        op.f("ix_sor_support_queues_created_at"), table_name="sor_support_queues"
    )
    op.drop_table("sor_support_queues")
    op.drop_index(
        "ix_unq_sor_support_messages_ext_id_org_id", table_name="sor_support_messages"
    )
    op.drop_index(
        "ix_sor_support_messages_source_ticket_created",
        table_name="sor_support_messages",
    )
    op.drop_index(
        op.f("ix_sor_support_messages_source_id"), table_name="sor_support_messages"
    )
    op.drop_index(
        op.f("ix_sor_support_messages_record_id"), table_name="sor_support_messages"
    )
    op.drop_index(
        op.f("ix_sor_support_messages_organization_id"),
        table_name="sor_support_messages",
    ...
    op.drop_index(
        op.f("ix_sor_support_messages_created_at"), table_name="sor_support_messages"
    )
    op.drop_table("sor_support_messages")
    op.drop_index(
        "ix_unq_sor_support_inboxes_ext_id_org_id", table_name="sor_support_inboxes"
    )
    op.drop_index(
        "ix_sor_support_inboxes_source_name", table_name="sor_support_inboxes"
    )
    op.drop_index(
        op.f("ix_sor_support_inboxes_source_id"), table_name="sor_support_inboxes"
    )
    op.drop_index(
        op.f("ix_sor_support_inboxes_record_id"), table_name="sor_support_inboxes"
    )
    op.drop_index(
        op.f("ix_sor_support_inboxes_organization_id"), table_name="sor_support_inboxes"
    )
    op.drop_index(
        op.f("ix_sor_support_inboxes_created_at"), table_name="sor_support_inboxes"
    )
    op.drop_table("sor_support_inboxes")
    op.drop_index(
        "ix_unq_sor_support_customers_ext_id_org_id", table_name="sor_support_customers"
    )
    op.drop_index(
        "ix_sor_support_customers_source_name", table_name="sor_support_customers"
    )
    op.drop_index(
        op.f("ix_sor_support_customers_source_id"), table_name="sor_support_customers"
    )
    op.drop_index(
        "ix_sor_support_customers_source_email", table_name="sor_support_customers"
    )
    op.drop_index(
        "ix_sor_support_customers_source_company", table_name="sor_support_customers"
    )
    op.drop_index(
        op.f("ix_sor_support_customers_record_id"), table_name="sor_support_customers"
    )
    op.drop_index(
        op.f("ix_sor_support_customers_organization_id"),
        table_name="sor_support_customers",
    )
    op.drop_index(
        op.f("ix_sor_support_customers_created_at"), table_name="sor_support_customers"
    )
    op.drop_table("sor_support_customers")
    op.drop_index(
        "ix_unq_sor_support_attachments_ext_id_org_id",
        table_name="sor_support_attachments",
    )
    op.drop_index(
        "ix_sor_support_attachments_source_ticket", table_name="sor_support_attachments"
    )
    op.drop_index(
        "ix_sor_support_attachments_source_message",
        table_name="sor_support_attachments",
    ...
    op.drop_table("sor_support_attachments")
    op.drop_index(
        "ix_unq_sor_support_agents_ext_id_org_id", table_name="sor_support_agents"
    )
    op.drop_index("ix_sor_support_agents_source_name", table_name="sor_support_agents")
    op.drop_index(
        op.f("ix_sor_support_agents_source_id"), table_name="sor_support_agents"
    )
    op.drop_index(
        "ix_sor_support_agents_source_active", table_name="sor_support_agents"
    )
    op.drop_index(
        op.f("ix_sor_support_agents_record_id"), table_name="sor_support_agents"
    )
    op.drop_index(
        op.f("ix_sor_support_agents_organization_id"), table_name="sor_support_agents"
    )
    op.drop_index(
        op.f("ix_sor_support_agents_created_at"), table_name="sor_support_agents"
    )
    op.drop_table("sor_support_agents")
    op.drop_index(
        "ix_unq_sor_relation_intents_ext_id_org_id", table_name="sor_relation_intents"
    )
    op.drop_index(
        "ix_sor_relation_intents_to_endpoint", table_name="sor_relation_intents"
    )
    op.drop_index(
        op.f("ix_sor_relation_intents_state"), table_name="sor_relation_intents"
    )
    op.drop_index(
        "ix_sor_relation_intents_source_state", table_name="sor_relation_intents"
    )
    op.drop_index(
        op.f("ix_sor_relation_intents_source_id"), table_name="sor_relation_intents"
    )
    op.drop_index(
        op.f("ix_sor_relation_intents_origin_record_id"),
    ...
    op.drop_index(
        "ix_sor_relation_intents_from_endpoint", table_name="sor_relation_intents"
    )
    op.drop_index(
        op.f("ix_sor_relation_intents_created_at"), table_name="sor_relation_intents"
    )
    op.drop_table("sor_relation_intents")
    op.drop_index(
        "uq_sor_record_relations_external_id",
        table_name="sor_record_relations",
        postgresql_where=sa.text("external_relation_id IS NOT NULL"),
    )
    op.drop_index(
        "ix_unq_sor_record_relations_ext_id_org_id", table_name="sor_record_relations"
    )
    op.drop_index(
        op.f("ix_sor_record_relations_to_record_id"), table_name="sor_record_relations"
    )
    op.drop_index(
        op.f("ix_sor_record_relations_source_id"), table_name="sor_record_relations"
    )
    op.drop_index(
        op.f("ix_sor_record_relations_organization_id"),
        table_name="sor_record_relations",
    ...
    op.drop_index(
        op.f("ix_sor_record_relations_created_at"), table_name="sor_record_relations"
    )
    op.drop_table("sor_record_relations")
    op.drop_index(
        "ix_unq_sor_knowledge_versions_ext_id_org_id",
        table_name="sor_knowledge_versions",
    )
    op.drop_index(
        op.f("ix_sor_knowledge_versions_source_id"), table_name="sor_knowledge_versions"
    )
    op.drop_index(
        "ix_sor_knowledge_versions_source_document_created",
        table_name="sor_knowledge_versions",
    ...
    op.drop_index(
        op.f("ix_sor_knowledge_versions_record_id"), table_name="sor_knowledge_versions"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_versions_organization_id"),
        table_name="sor_knowledge_versions",
    ...
    op.drop_table("sor_knowledge_versions")
    op.drop_index(
        "ix_unq_sor_knowledge_spaces_ext_id_org_id", table_name="sor_knowledge_spaces"
    )
    op.drop_index(
        "ix_sor_knowledge_spaces_source_name", table_name="sor_knowledge_spaces"
    )
    op.drop_index(
        "ix_sor_knowledge_spaces_source_kind", table_name="sor_knowledge_spaces"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_spaces_source_id"), table_name="sor_knowledge_spaces"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_spaces_record_id"), table_name="sor_knowledge_spaces"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_spaces_organization_id"),
        table_name="sor_knowledge_spaces",
    )
    op.drop_index(
        op.f("ix_sor_knowledge_spaces_created_at"), table_name="sor_knowledge_spaces"
    )
    op.drop_table("sor_knowledge_spaces")
    op.drop_index(
        "ix_unq_sor_knowledge_properties_ext_id_org_id",
        table_name="sor_knowledge_properties",
    )
    op.drop_index(
        op.f("ix_sor_knowledge_properties_source_id"),
        table_name="sor_knowledge_properties",
    ...
    op.drop_table("sor_knowledge_properties")
    op.drop_index(
        "ix_unq_sor_knowledge_documents_ext_id_org_id",
        table_name="sor_knowledge_documents",
    )
    op.drop_index(
        "ix_sor_knowledge_documents_source_updated",
        table_name="sor_knowledge_documents",
    ...
    op.drop_index(
        "ix_sor_knowledge_documents_source_title", table_name="sor_knowledge_documents"
    )
    op.drop_index(
        "ix_sor_knowledge_documents_source_space", table_name="sor_knowledge_documents"
    )
    op.drop_index(
        "ix_sor_knowledge_documents_source_parent", table_name="sor_knowledge_documents"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_documents_source_id"),
        table_name="sor_knowledge_documents",
    ...
    op.drop_table("sor_knowledge_documents")
    op.drop_index(
        "ix_unq_sor_knowledge_blocks_ext_id_org_id", table_name="sor_knowledge_blocks"
    )
    op.drop_index(
        "ix_sor_knowledge_blocks_source_parent", table_name="sor_knowledge_blocks"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_blocks_source_id"), table_name="sor_knowledge_blocks"
    )
    op.drop_index(
        "ix_sor_knowledge_blocks_source_document_position",
        table_name="sor_knowledge_blocks",
    )
    op.drop_index(
        op.f("ix_sor_knowledge_blocks_record_id"), table_name="sor_knowledge_blocks"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_blocks_organization_id"),
        table_name="sor_knowledge_blocks",
    ...
    op.drop_index(
        op.f("ix_sor_knowledge_blocks_created_at"), table_name="sor_knowledge_blocks"
    )
    op.drop_table("sor_knowledge_blocks")
    op.drop_index(
        "ix_unq_sor_knowledge_authors_ext_id_org_id", table_name="sor_knowledge_authors"
    )
    op.drop_index(
        "ix_sor_knowledge_authors_source_name", table_name="sor_knowledge_authors"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_authors_source_id"), table_name="sor_knowledge_authors"
    )
    op.drop_index(
        "ix_sor_knowledge_authors_source_email", table_name="sor_knowledge_authors"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_authors_record_id"), table_name="sor_knowledge_authors"
    )
    op.drop_index(
        op.f("ix_sor_knowledge_authors_organization_id"),
        table_name="sor_knowledge_authors",
    )
    op.drop_index(
        op.f("ix_sor_knowledge_authors_created_at"), table_name="sor_knowledge_authors"
    )
    op.drop_table("sor_knowledge_authors")
    op.drop_index(
        "ix_unq_sor_knowledge_attachments_ext_id_org_id",
        table_name="sor_knowledge_attachments",
    )
    op.drop_index(
        "ix_sor_knowledge_attachments_source_name",
        table_name="sor_knowledge_attachments",
    ...
    op.drop_table("sor_knowledge_attachments")
    op.drop_index(
        "ix_unq_sor_custom_field_values_ext_id_org_id",
        table_name="sor_custom_field_values",
    )
    op.drop_index(
        "ix_sor_custom_field_values_timestamp", table_name="sor_custom_field_values"
    )
    op.drop_index(
        "ix_sor_custom_field_values_text", table_name="sor_custom_field_values"
    )
    op.drop_index(
        "ix_sor_custom_field_values_string_array",
        table_name="sor_custom_field_values",
    ...
    op.drop_index(
        "ix_sor_custom_field_values_decimal", table_name="sor_custom_field_values"
    )
    op.drop_index(
        "ix_sor_custom_field_values_date", table_name="sor_custom_field_values"
    )
    op.drop_index(
        op.f("ix_sor_custom_field_values_created_at"),
        table_name="sor_custom_field_values",
    ...
    op.drop_index(
        "ix_sor_custom_field_values_boolean", table_name="sor_custom_field_values"
    )
    op.drop_table("sor_custom_field_values")
    op.drop_index("ix_unq_sor_crm_deals_ext_id_org_id", table_name="sor_crm_deals")
    op.drop_index("ix_sor_crm_deals_source_state", table_name="sor_crm_deals")
    op.drop_index("ix_sor_crm_deals_source_stage", table_name="sor_crm_deals")
    op.drop_index("ix_sor_crm_deals_source_owner", table_name="sor_crm_deals")
    op.drop_index(op.f("ix_sor_crm_deals_source_id"), table_name="sor_crm_deals")
    op.drop_index("ix_sor_crm_deals_source_close", table_name="sor_crm_deals")
    op.drop_index(op.f("ix_sor_crm_deals_record_id"), table_name="sor_crm_deals")
    op.drop_index(op.f("ix_sor_crm_deals_organization_id"), table_name="sor_crm_deals")
    op.drop_index(op.f("ix_sor_crm_deals_created_at"), table_name="sor_crm_deals")
    op.drop_table("sor_crm_deals")
    op.drop_index(
        "ix_unq_sor_crm_contacts_ext_id_org_id", table_name="sor_crm_contacts"
    )
    op.drop_index("ix_sor_crm_contacts_source_owner", table_name="sor_crm_contacts")
    op.drop_index("ix_sor_crm_contacts_source_lifecycle", table_name="sor_crm_contacts")
    op.drop_index(op.f("ix_sor_crm_contacts_source_id"), table_name="sor_crm_contacts")
    op.drop_index("ix_sor_crm_contacts_source_email", table_name="sor_crm_contacts")
    op.drop_index(op.f("ix_sor_crm_contacts_record_id"), table_name="sor_crm_contacts")
    op.drop_index(
        op.f("ix_sor_crm_contacts_organization_id"), table_name="sor_crm_contacts"
    )
    op.drop_index(op.f("ix_sor_crm_contacts_created_at"), table_name="sor_crm_contacts")
    op.drop_table("sor_crm_contacts")
    op.drop_index(
        "ix_unq_sor_crm_companies_ext_id_org_id", table_name="sor_crm_companies"
    )
    op.drop_index("ix_sor_crm_companies_source_owner", table_name="sor_crm_companies")
    op.drop_index(
        "ix_sor_crm_companies_source_industry", table_name="sor_crm_companies"
    )
    op.drop_index(
        op.f("ix_sor_crm_companies_source_id"), table_name="sor_crm_companies"
    )
    op.drop_index("ix_sor_crm_companies_source_domain", table_name="sor_crm_companies")
    op.drop_index(
        op.f("ix_sor_crm_companies_record_id"), table_name="sor_crm_companies"
    )
    op.drop_index(
        op.f("ix_sor_crm_companies_organization_id"), table_name="sor_crm_companies"
    )
    op.drop_index(
        op.f("ix_sor_crm_companies_created_at"), table_name="sor_crm_companies"
    )
    op.drop_table("sor_crm_companies")
    op.drop_index(
        "ix_unq_sor_crm_activities_ext_id_org_id", table_name="sor_crm_activities"
    )
    op.drop_index(
        "ix_sor_crm_activities_source_occurred", table_name="sor_crm_activities"
    )
    op.drop_index("ix_sor_crm_activities_source_kind", table_name="sor_crm_activities")
    op.drop_index(
        op.f("ix_sor_crm_activities_source_id"), table_name="sor_crm_activities"
    )
    op.drop_index(
        op.f("ix_sor_crm_activities_record_id"), table_name="sor_crm_activities"
    )
    op.drop_index(
        op.f("ix_sor_crm_activities_organization_id"), table_name="sor_crm_activities"
    )
    op.drop_index(
        op.f("ix_sor_crm_activities_created_at"), table_name="sor_crm_activities"
    )
    op.drop_table("sor_crm_activities")
    op.drop_index("ix_unq_sor_commands_ext_id_org_id", table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_target_record_id"), table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_state"), table_name="sor_commands")
    op.drop_index("ix_sor_commands_source_state", table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_source_id"), table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_source_grant_id"), table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_organization_id"), table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_created_at"), table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_agent_run_id"), table_name="sor_commands")
    op.drop_index("ix_sor_commands_agent_run", table_name="sor_commands")
    op.drop_index(op.f("ix_sor_commands_agent_id"), table_name="sor_commands")
    op.drop_table("sor_commands")
    ...
    op.drop_index(
        op.f("ix_voice_sessions_user_session_id"), table_name="voice_sessions"
    )
    ...
    op.drop_index("ix_unq_sor_sync_runs_ext_id_org_id", table_name="sor_sync_runs")
    op.drop_index(op.f("ix_sor_sync_runs_stream_id"), table_name="sor_sync_runs")
    op.drop_index(op.f("ix_sor_sync_runs_state"), table_name="sor_sync_runs")
    op.drop_index(op.f("ix_sor_sync_runs_source_id"), table_name="sor_sync_runs")
    op.drop_index("ix_sor_sync_runs_source_created", table_name="sor_sync_runs")
    op.drop_index(op.f("ix_sor_sync_runs_organization_id"), table_name="sor_sync_runs")
    op.drop_index(
        op.f("ix_sor_sync_runs_mapping_revision_id"), table_name="sor_sync_runs"
    )
    op.drop_index(op.f("ix_sor_sync_runs_generation_id"), table_name="sor_sync_runs")
    op.drop_index(op.f("ix_sor_sync_runs_created_at"), table_name="sor_sync_runs")
    op.drop_table("sor_sync_runs")
    op.drop_index("ix_unq_sor_records_ext_id_org_id", table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_tombstoned_at"), table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_source_updated_at"), table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_source_id"), table_name="sor_records")
    op.drop_index(
        "ix_sor_records_search_vector", table_name="sor_records", postgresql_using="gin"
    )
    op.drop_index(op.f("ix_sor_records_projected_at"), table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_profile"), table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_organization_id"), table_name="sor_records")
    op.drop_index(
        "ix_sor_records_org_source_profile_entity_projected_live",
        table_name="sor_records",
        postgresql_where=sa.text("tombstoned_at IS NULL AND deleted = false"),
    )
    op.drop_index(
        "ix_sor_records_org_profile_entity_projected_live",
    ...
    op.drop_index(op.f("ix_sor_records_mapping_revision_id"), table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_human_external_key"), table_name="sor_records")
    op.drop_index(op.f("ix_sor_records_created_at"), table_name="sor_records")
    op.drop_index(
        op.f("ix_sor_records_canonical_entity_kind"), table_name="sor_records"
    )
    op.drop_index(
        "ix_sor_records_agent_search_vector",
        table_name="sor_records",
    ...
    op.drop_table("sor_records")
    op.drop_index(
        "ix_unq_sor_field_mappings_ext_id_org_id", table_name="sor_field_mappings"
    )
    op.drop_index(
        op.f("ix_sor_field_mappings_source_id"), table_name="sor_field_mappings"
    )
    op.drop_index(
        op.f("ix_sor_field_mappings_organization_id"), table_name="sor_field_mappings"
    )
    op.drop_index(
        op.f("ix_sor_field_mappings_mapping_revision_id"),
        table_name="sor_field_mappings",
    )
    op.drop_index(
        op.f("ix_sor_field_mappings_custom_field_definition_id"),
        table_name="sor_field_mappings",
    ...
    op.drop_index(
        op.f("ix_sor_field_mappings_created_at"), table_name="sor_field_mappings"
    )
    op.drop_table("sor_field_mappings")
    op.drop_index(
        "ix_unq_scheduler_schedule_revisions_ext_id_org_id",
    ...
    op.drop_index(
        op.f("ix_telephony_calls_user_session_id"), table_name="telephony_calls"
    )
    op.drop_index(
    ...
    op.drop_index(
        "uq_sor_mapping_revisions_active_source",
        table_name="sor_mapping_revisions",
        postgresql_where=sa.text("state = 'ACTIVE' AND deleted = false"),
    )
    op.drop_index(
        "ix_unq_sor_mapping_revisions_ext_id_org_id", table_name="sor_mapping_revisions"
    )
    op.drop_index(
        op.f("ix_sor_mapping_revisions_state"), table_name="sor_mapping_revisions"
    )
    op.drop_index(
        op.f("ix_sor_mapping_revisions_source_schema_revision_id"),
        table_name="sor_mapping_revisions",
    ...
    op.drop_index(
        op.f("ix_sor_mapping_revisions_source_id"), table_name="sor_mapping_revisions"
    )
    op.drop_index(
        op.f("ix_sor_mapping_revisions_organization_id"),
        table_name="sor_mapping_revisions",
    ...
    op.drop_index(
        op.f("ix_sor_mapping_revisions_created_at"), table_name="sor_mapping_revisions"
    )
    op.drop_table("sor_mapping_revisions")
    op.drop_index(
        op.f("ix_sor_agent_revision_source_grants_source_id"),
        table_name="sor_agent_revision_source_grants",
    )
    op.drop_index(
        op.f("ix_sor_agent_revision_source_grants_source_grant_id"),
        table_name="sor_agent_revision_source_grants",
    ...
    op.drop_table("sor_agent_revision_source_grants")
    op.drop_index(
    ...
    op.drop_index(
        "ix_unq_sor_webhook_receipts_ext_id_org_id", table_name="sor_webhook_receipts"
    )
    op.drop_index(
        "ix_sor_webhook_receipts_source_state", table_name="sor_webhook_receipts"
    )
    op.drop_index(
        op.f("ix_sor_webhook_receipts_source_id"), table_name="sor_webhook_receipts"
    )
    op.drop_index(
        op.f("ix_sor_webhook_receipts_raw_body_expires_at"),
        table_name="sor_webhook_receipts",
    )
    op.drop_index(
        op.f("ix_sor_webhook_receipts_organization_id"),
        table_name="sor_webhook_receipts",
    ...
    op.drop_index(
        op.f("ix_sor_webhook_receipts_created_at"), table_name="sor_webhook_receipts"
    )
    op.drop_table("sor_webhook_receipts")
    op.drop_index(
        "uq_sor_sync_generations_active_source",
    ...
    op.drop_index(
        "ix_unq_sor_sync_generations_ext_id_org_id", table_name="sor_sync_generations"
    )
    op.drop_index(
        op.f("ix_sor_sync_generations_state"), table_name="sor_sync_generations"
    )
    op.drop_index(
        op.f("ix_sor_sync_generations_source_id"), table_name="sor_sync_generations"
    )
    op.drop_index(
        "ix_sor_sync_generations_source_created", table_name="sor_sync_generations"
    )
    op.drop_index(
        op.f("ix_sor_sync_generations_organization_id"),
        table_name="sor_sync_generations",
    ...
    op.drop_index(
        "ix_sor_sync_generations_org_state", table_name="sor_sync_generations"
    )
    op.drop_index(
        op.f("ix_sor_sync_generations_created_at"), table_name="sor_sync_generations"
    )
    op.drop_table("sor_sync_generations")
    op.drop_index(
        "ix_unq_sor_source_streams_ext_id_org_id", table_name="sor_source_streams"
    )
    op.drop_index(
        op.f("ix_sor_source_streams_source_id"), table_name="sor_source_streams"
    )
    op.drop_index(
        op.f("ix_sor_source_streams_organization_id"), table_name="sor_source_streams"
    )
    op.drop_index(
        op.f("ix_sor_source_streams_next_due_at"), table_name="sor_source_streams"
    )
    op.drop_index("ix_sor_source_streams_due", table_name="sor_source_streams")
    op.drop_index(
        op.f("ix_sor_source_streams_created_at"), table_name="sor_source_streams"
    )
    op.drop_table("sor_source_streams")
    op.drop_index(
        "uq_sor_source_grants_agent_source_active",
        table_name="sor_source_grants",
        postgresql_where=sa.text("deleted = false"),
    )
    op.drop_index(
        "ix_unq_sor_source_grants_ext_id_org_id", table_name="sor_source_grants"
    )
    op.drop_index(
        op.f("ix_sor_source_grants_source_id"), table_name="sor_source_grants"
    )
    op.drop_index(
        op.f("ix_sor_source_grants_organization_id"), table_name="sor_source_grants"
    )
    op.drop_index(
        op.f("ix_sor_source_grants_created_at"), table_name="sor_source_grants"
    )
    op.drop_index(op.f("ix_sor_source_grants_agent_id"), table_name="sor_source_grants")
    op.drop_table("sor_source_grants")
    op.drop_index(
        "ix_unq_sor_schema_revisions_ext_id_org_id", table_name="sor_schema_revisions"
    )
    op.drop_index(
        op.f("ix_sor_schema_revisions_source_id"), table_name="sor_schema_revisions"
    )
    op.drop_index(
        op.f("ix_sor_schema_revisions_schema_hash"), table_name="sor_schema_revisions"
    )
    op.drop_index(
        op.f("ix_sor_schema_revisions_organization_id"),
        table_name="sor_schema_revisions",
    )
    op.drop_index(
        op.f("ix_sor_schema_revisions_created_at"), table_name="sor_schema_revisions"
    )
    op.drop_table("sor_schema_revisions")
    op.drop_index(
        "ix_unq_sor_custom_field_definitions_ext_id_org_id",
        table_name="sor_custom_field_definitions",
    )
    op.drop_index(
        "ix_sor_custom_fields_source_active",
        table_name="sor_custom_field_definitions",
    ...
    op.drop_table("sor_custom_field_definitions")
    op.drop_index(
        "ix_unq_sor_custom_datasets_ext_id_org_id", table_name="sor_custom_datasets"
    )
    op.drop_index(
        op.f("ix_sor_custom_datasets_source_id"), table_name="sor_custom_datasets"
    )
    op.drop_index(
        op.f("ix_sor_custom_datasets_organization_id"), table_name="sor_custom_datasets"
    )
    op.drop_index("ix_sor_custom_datasets_org_source", table_name="sor_custom_datasets")
    op.drop_index(
        op.f("ix_sor_custom_datasets_created_at"), table_name="sor_custom_datasets"
    )
    op.drop_table("sor_custom_datasets")
    ...
    op.drop_index(
        "uq_sor_sources_webhook_endpoint_token_hash",
        table_name="sor_sources",
        postgresql_where=sa.text("webhook_endpoint_token_hash IS NOT NULL"),
    )
    op.drop_index("ix_unq_sor_sources_ext_id_org_id", table_name="sor_sources")
    op.drop_index(
        op.f("ix_sor_sources_webhook_subscription_expires_at"), table_name="sor_sources"
    )
    op.drop_index(op.f("ix_sor_sources_vendor_key"), table_name="sor_sources")
    op.drop_index(op.f("ix_sor_sources_state"), table_name="sor_sources")
    op.drop_index(op.f("ix_sor_sources_profile"), table_name="sor_sources")
    op.drop_index(op.f("ix_sor_sources_organization_id"), table_name="sor_sources")
    op.drop_index("ix_sor_sources_org_profile_state", table_name="sor_sources")
    op.drop_index(
        op.f("ix_sor_sources_last_successful_sync_at"), table_name="sor_sources"
    )
    op.drop_index(
        op.f("ix_sor_sources_external_connection_id"), table_name="sor_sources"
    )
    op.drop_index(op.f("ix_sor_sources_created_at"), table_name="sor_sources")
    op.drop_table("sor_sources")
    op.drop_index(
        "uq_sor_connectors_webhook_endpoint_key_active",
    ...
    op.drop_index("ix_unq_sor_connectors_ext_id_org_id", table_name="sor_connectors")
    op.drop_index(op.f("ix_sor_connectors_vendor_key"), table_name="sor_connectors")
    op.drop_index(op.f("ix_sor_connectors_profile"), table_name="sor_connectors")
    op.drop_index(
        op.f("ix_sor_connectors_organization_id"), table_name="sor_connectors"
    )
    op.drop_index("ix_sor_connectors_org_profile_vendor", table_name="sor_connectors")
    op.drop_index(
        op.f("ix_sor_connectors_external_connection_id"), table_name="sor_connectors"
    )
    op.drop_index(op.f("ix_sor_connectors_created_at"), table_name="sor_connectors")
    op.drop_table("sor_connectors")
    op.drop_index(
        "uq_memory_reindex_jobs_active_config",
    ...
    op.drop_index(
        "ix_unq_integration_v2_connection_links_ext_id_org_id",
        table_name="integration_v2_connection_links",
    )
    op.drop_index(
        op.f("ix_integration_v2_connection_links_organization_id"),
        table_name="integration_v2_connection_links",
    ...
    op.drop_table("integration_v2_connection_links")
    op.drop_index(
        op.f("ix_event_inbox_receipts_organization_id"),
    ...
    op.drop_index(op.f("ix_agent_agents_voice_config_id"), table_name="agent_agents")
    op.drop_index(
    ...
    op.drop_index(
        op.f("ix_voice_configs_tts_provider_config_id"), table_name="voice_configs"
    )
    op.drop_index(
        op.f("ix_voice_configs_stt_provider_config_id"), table_name="voice_configs"
    )
    op.drop_index(
        op.f("ix_voice_configs_storage_provider_config_id"), table_name="voice_configs"
    )
    op.drop_index(
        op.f("ix_voice_configs_realtime_provider_config_id"), table_name="voice_configs"
    )
    op.drop_index(op.f("ix_voice_configs_organization_id"), table_name="voice_configs")
    op.drop_index(op.f("ix_voice_configs_created_at"), table_name="voice_configs")
    op.drop_index("ix_unq_voice_configs_ext_id_org_id", table_name="voice_configs")
    ...
    op.drop_index("ix_user_sessions_org_state_activity", table_name="user_sessions")
    op.drop_index("ix_user_sessions_org_started", table_name="user_sessions")
    op.drop_index("ix_user_sessions_org_contact_started", table_name="user_sessions")
    ...
    op.drop_index("ix_unq_user_sessions_ext_id_org_id", table_name="user_sessions")
    ...
    op.drop_index(
        "ix_unq_connection_external_connections_ext_id_org_id",
        table_name="connection_external_connections",
    )
    op.drop_index(
        op.f("ix_connection_external_connections_vendor_key"),
        table_name="connection_external_connections",
    ...
    op.drop_table("connection_external_connections")
    ...
    op.drop_index("ix_event_outbox_org_correlation_occurred", table_name="event_outbox")
    ...
    op.drop_index(op.f("ix_agent_runs_user_session_id"), table_name="agent_runs")
